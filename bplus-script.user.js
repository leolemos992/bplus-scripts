...
// @version      15.0.0
// @description  Correção para a renderização de chats, com inicialização segura para evitar conflitos com a página.
...
(function() {
    ...
    const CONFIG = {
        SCRIPT_VERSION: GM_info.script.version || '15.0.0',
    ...
    function injectStyles() {
        ...
        GM_addStyle(`
            ...
            .crx-filter-tabs { display: flex; flex-shrink: 0; overflow-x: auto; overflow-y: hidden; height: 48px; align-items: center; padding: 0 8px; background-color: #fff; border-bottom: 1px solid #e0e0e0; }
            ...
            .crx-group-header { display: flex; align-items: center; justify-content: space-between; padding: 12px 12px 4px; font-size: 13px; font-weight: 600; color: #6c757d; text-transform: uppercase; position: sticky; top: 0; background: inherit; z-index: 10; cursor: pointer; user-select: none; border-bottom: 1px solid #e0e0e0; }
            .dark .crx-group-header { color: #a0a0b0; border-bottom-color: #3e374e; }
            .crx-group-header.group-waiting { color: #e67e22; }
    ...
    function parseChatItemData(itemElement, index) {
        ...
        const isWaiting = Array.from(itemElement.querySelectorAll(SELECTORS.statusTag))
                               .some(tag => tag.textContent?.includes('Aguardando'));
    ...
    function updateTabsLayout(waiting, my, other) {
        ...
        tabsContainer.querySelectorAll('.crx-filter-tab').forEach(t => t.addEventListener('click', () => { STATE.activeFilter = t.dataset.filter; updateFullUI(); }));
    ...
    async function initialize() {
        ...
        if (!mainSection) {
            log("ERRO: Container principal não encontrado.");
            return;
        }

        // PASSO 2: Renderiza a nossa estrutura (ainda vazia).
        renderInitialShell(mainSection);
        
        // PASSO 3: Ativa a regra de CSS para esconder a lista original.
        document.body.classList.add('bplus-active');

        // PASSO 4: Popula nossa UI pela primeira vez.
        updateChatList();

        // PASSO 5: Começa a observar mudanças para manter a lista atualizada.
